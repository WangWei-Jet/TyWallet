...
// must use below cuz of http://stackoverflow
// .com/questions/33502939/android-util-pair-holds-string-as-parameters-in-androidtest-but-null
// -in-unit
...
public class BitcoinjWrapper {
    ...
    public static String getNewPrivateKey(Boolean isTestnet) {
        NetworkParameters params = BitcoinjWrapper.getNetwork(isTestnet);
        ECKey key = new ECKey();
    ...
    public static String getAddressFromPublicKey(String publicKey, Boolean isTestnet) {
        ...
        return addressKey.toAddress(BitcoinjWrapper.getNetwork(isTestnet)).toString();
    }
    ...
    public static Boolean isBIP38EncryptedKey(String privateKey, Boolean isTestnet) {
        try {
            BIP38PrivateKey.fromBase58(BitcoinjWrapper.getNetwork(isTestnet), privateKey);
            return true;
        } catch (AddressFormatException name) {
            return false;
        }
    ...
    public static Boolean isValidAddress(String address, Boolean isTestnet) {
        try {
            ...
            if (StealthAddress.isStealthAddress(address, isTestnet)) {
                return true;
            }
            return false;
        }
...
//                tx.addOutput(new TransactionOutput(params, null, Coin.COIN, addr));
        ...
        for (int i = 0; i < hashes.size(); i++) {
            ...
            hash = BitcoinjWrapper.reverseHexString(hash);
