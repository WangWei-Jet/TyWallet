...
public class UpgradeUtil {
    ...
    public UpgradeUtil(TyWalletBtDriver tyWalletBtDriver) {
    ...
    public boolean upgradeDevice(byte[] file, UpgradeListener listener) {
        TYLog.d(TAG, "upgradeDevice invoked");
        boolean result = uD(file, listener);
        TYLog.d(TAG, "result:" + result);
        TYLog.d(TAG, "upgradeDevice invoked over");
        return result;
    }
    ...
    private boolean uD(byte[] file, UpgradeListener listener) {
        try {
            if (file == null) {
                TYLog.w(TAG, "upgrade file or the delegate is null");
                return false;
            }
            String pn = getDevicePN();
            if (pn == null) {
                TYLog.w(TAG, "failed fetching PN");
                return false;
            }
            ...
            if (pn.endsWith("71241")) {
                if (!saveFile41(file)) {
                    TYLog.w(TAG, "failed saving upgrade file");
                    if (listener != null) {
...
//            if (!compareHDVer()) {
//                TYLog.e(TAG, "hardware version not matched");
//                if (listener != null) {
...
//            if (!compareVersion()) {
//                TYLog.e(TAG, "version not matched");
//                listener.upgradeFail(UpgradeListener.COMPARE_VERSION_FAIL);
//                return false;
//            }

            if (Experimental()) {
                TYLog.d(TAG, "first step=>verification:success");
                if (clearAPP()) {
                    TYLog.d(TAG, "second step=>clear APP backup:success");
                    if (deviceDownload(listener)) {
                        TYLog.d(TAG, "third step=>download encrepted hardware program:success");
                        if (pn.endsWith("71249") || pn.endsWith("63250")
                                || pn.endsWith("63251")) {
                            if (downloadBackupArea49(listener)) {
                                TYLog.d(TAG, "fourth step=>download params for backup " +
                                        "area:success");
                                if (listener != null) {
                        ...
                        TYLog.d(TAG, "third step=>download encrepted hardware program:fail");
                    }
                } else {
                    if (listener != null) {
                        listener.upgradeFail(UpgradeListener.CLEAR_APP_FAIL);
                    }
                    TYLog.d(TAG, "second step=>clear APP backup:fail");
                }
            } else {
                if (listener != null) {
                    listener.upgradeFail(UpgradeListener.EXPERIMENTAL_FAIL);
                }
                TYLog.d(TAG, "first step=>verification:fail");
            }
        } catch (Exception e) {
            TYLog.e(TAG, "error occues trying to upgrade device", e);
            if (listener != null) {
    ...
    private boolean saveFile41(byte[] file) {
        ...
        if (file != null && file.length > 512) {
            ...
            signflag = file[373];
            TYLog.d("singflag", file[372] + "  " + file[373] + "  "
                    + file[374]);
            if (signflag == 0x01) {
                TYLog.d(TAG, "flag with sign");
                System.arraycopy(file, 512, fileData, 0, 256);
            }
            TYLog.d(TAG,"file header appLen:" + Utils.bytesToHexString(appLen, 4)
            ...
            TYLog.d(TAG,"encryptTransKey result of file header"
                            + Utils.bytesToHexString(encryptTransKey, 16));
            TYLog.d(TAG,"PN of file header:" + Utils.bytesToHexString(PN, 8)
                            + "\nversion："
    ...
    private boolean comparePN() {
        ...
        if (devicePN == null) {
            TYLog.d(TAG, "failed fetching device PN");
            return false;
        }
    ...
    public String getDevicePN() {
        TYLog.d(TAG, "getDevicePN invoked");
        String result = gDPN();
        TYLog.d(TAG, "result:" + result);
        TYLog.d(TAG, "getDevicePN invoked over");
        return result;
    }
    ...
    private String gDPN() {
        try {
            ...
            TYLog.d(TAG, "prepare to get device PN");
            ...
            if (ret > 0) {
                ...
                TYLog.d(TAG, "command interaction timeout!");
            }
        } catch (Exception e) {
            TYLog.e(TAG, "error occurs trying to get device PN",e);
        }
        return null;
    }
    ...
    private boolean compareHDVer() {
        ...
        if (HDVer == null) {
            TYLog.d(TAG, "failed fetching hardware version");
            return false;
        }
        ...
        if (hardVer != null) {
            ...
            HDVer = HDVer.replace(".", "");
            TYLog.e(TAG, "hardware version of device:" + HDVer);
    ...
    public String getHardwareVersion() {
        TYLog.d(TAG, "getHardwareVersion invoked");
        String result = gHV();
        TYLog.d(TAG, "result:" + result);
        TYLog.d(TAG, "getHardwareVersion invoked over");
        return result;
    }
    ...
    private boolean compareVersion() {
        ...
        if (deviceVersion == null) {
            TYLog.d(TAG, "failed fetching device version");
            return false;
        }
        ...
        if (AppVer != null) {
            ...
            TYLog.d(TAG, "version of file:" + fileVerASCII);
            ...
            TYLog.d(TAG, "version of device:" + deviceASCII);
            ...
            TYLog.d(TAG, deviceASCII.substring(1, 3) + "----"
    ...
    private boolean Experimental() {
        ...
        TYLog.d("Experimental", Utils.bytesToHexString(hex, hex.length));
        ...
        TYLog.d(TAG, "verification command:"
    ...
    public String getDeviceVersion() {
        TYLog.d(TAG, "getDeviceVersion invoked");
        String result = gDV();
        TYLog.d(TAG, "result:" + result);
        TYLog.d(TAG, "getDeviceVersion invoked over");
        return result;
    }
    ...
    private String gDV() {
        try {
            ...
            byte[] response = new byte[300];
            TYLog.d(TAG, "prepare to get device version");
            int ret = tyWalletBtDriver.transCommandAPDU(cmd_send, cmd_send.length,
                    response, 3000);
            if (ret > 0) {
                byte[] res = new byte[ret];
            ...
            TYLog.e(TAG, "error occurs trying to get device version",e);
        }
        return null;
    }
        ...
        if (fileContent != null) {
            ...
            if (count != 0) {
                for (int i = 0; i < count; i++) {
                    cmd[4] = (byte) (len + 6);
                    TYLog.d(TAG, "offset:" + startAdress + "");
            ...
            TYLog.d("count1", count1 + "" + "count*len" + count * len);
            ...
            if (count1 != 0) {
                ...
                TYLog.d(TAG, "send last downloaded data:"
                        + Utils.bytesToHexString(cmd, cmd.length));
                byte[] res = new byte[300];
                int ret = tyWalletBtDriver.transCommandAPDU(cmd, cmd.length, res, 3000);
                if (ret > 0 && res[ret - 1] == (byte) 0x00
                    ...
                    if (ret > 0) {
                        TYLog.d(TAG, "fail code of last step download"
                                + Utils.bytesToHexString(res, ret));
                        if (res[ret - 1] == (byte) 0x82
                                && res[ret - 2] == (byte) 0x6A) {
                            if (listener != null) {
                ...
                TYLog.d(TAG, "every time 248 bytes and no left:success");
                return true;
            }
        }
        if (listener != null) {
    ...
    private boolean downloadBackupArea49(UpgradeListener listener) {
        ...
        if (signflag == 0x00) {
            ...
            if (applen != null && xor32 != null & key3 != null) {
                ...
                TYLog.d(TAG, "hardware verification total:" + c32 + "");
                ...
                if (ret > 0) {
                    TYLog.d(TAG, "download backup area:"
                            + Utils.bytesToHexString(res, ret));
                    if (res[ret - 1] == (byte) 0x82
                            && res[ret - 2] == (byte) 0x6A) {
                        if (listener != null) {
        ...
        if (signflag == 0x01) {
            ...
            if (applen != null && xor32 != null & key3 != null) {
                ...
                long c32 = calcXor32(fileContent, fileContent.length);
                TYLog.d(TAG, "enc hardware total:" + c32 + "");
